	...
	for(pop=ind=0;pop<npop;pop++)
	  {
	    for(j=0; j<inconfig[pop];j++,ind++) 
	      {
		sprintf(seglst[0].ptree[ind].name,"%i_pop",pop);
	      }
	  }
	nnodes[0] = nsam - 1 ;
	nchrom=nsam;
...
/* Main loop */
	...
	if( ( ( eflag == 0) && (nextevent != NULL))|| ( (nextevent != NULL) &&  ( (t+tmin) >=  nextevent->time)) ) {
		...
		case 'm' :
		  ...
		  // peter beerli
		  popsplitcounter += 1;
		  fprintf(stderr,"\nPopulation split %li: Time=%f Total lineages: %i migrations: %li\n",popsplitcounter, t, nchrom,migcounter);
	...
	*pnsegs = nsegs ;
	...
	if(nsegs>1)
	  {
	    int nsi;
	    for (nsi=1;nsi<nsegs;nsi++)
	      {
		for(pop=ind=0;pop<npop;pop++)
		  {
		    for(j=0; j<inconfig[pop];j++,ind++) 
		      {
			sprintf(seglst[nsi].ptree[ind].name,"%i_pop",pop);
		      }
		  }
	      }
	  }
	if (popsplitcounter < 1)
	  fprintf(stderr,"\nPopulation NOT split %li: Time=%f Total lineages: 0 migrations: %li\n",popsplitcounter, t, migcounter);
	free(config); 
	free( size ) ;
	free( alphag );
	free( tlast );
