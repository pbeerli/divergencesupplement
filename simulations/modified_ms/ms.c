...
gensam( char **list, double *pprobss, double *ptmrca, double *pttot ) 
{
	...
	double theta, es ;
	int nsam, mfreq ;
	void prtree( struct node *ptree, int nsam, double scaling);
	...
	if( pars.mp.treeflag ) {
		...
		if(pars.cp.pbout==1)
		  {
		    ...
		    for(pop=0;pop<pars.cp.npop;pop++)
		      {
			fprintf(stdout, "%i ", pars.cp.config[pop]);
		      }
		    fprintf(stdout, "\n#%i %i 2.0\n", nsegs, nsites);
		    for( seg=0, k=0; k<nsegs; seg=seglst[seg].next, k++) {
		      {
			fprintf(stdout, "# rate among sites for locus %i (1.000000)\n", seg);
			fprintf(stdout, "#");
			int site;
	    ...
	    for( seg=0, k=0; k<nsegs; seg=seglst[seg].next, k++) {
	      ...
	      // peter beerli adaptation
	      double mutation = pars.cp.mutationrate;
	      fprintf(stdout,"#$Locus %i\n#$%20.20f {%f/%d=%f}\n",seg, mutation,pars.mp.theta , pars.cp.nsites,pars.mp.theta / pars.cp.nsites);
	      prtree( seglst[seg].ptree, nsam, pars.mp.theta / pars.cp.nsites ) ;
	      fprintf(stderr,"tmrca: %f\n",seglst[seg].ptree[2*nsam-1].time);
	...
	for( i=0; i<nsam; i++) 
	  {
	    (ptree+i)->ndes = 1 ;
	    //(ptree+i)->pop = z+1;
	    sprintf((ptree + i)->name, "%i_%i", z+1,i);
	    //printf("ndes_setup: %i %i %s\\n",i, z, (ptree+i)->name);
	    if(ind++ >= pars.cp.config[z])
	      {
		z++;
		ind=0;
	      }
	...
	while( arg < argc ){
		...
		switch ( argv[arg][1] ){
			...
			case 't' : 
				...
				break;
				// peter beerli addition
		case 'q':
		  // peter beerli addition to get the number of sites for the migdata simulator
		  // to specify the mutation rate per site
		  arg++;
		  argcheck(arg,argc, argv);
		  pars.cp.mutationrate = atof(  argv[arg++] );
		  break;
		case 'l':
		  // peter beerli addition to get the number of sites for the migdata simulator
		  arg++;
...
fprintf(stderr,"\t -M          (Output gene tree compatible with MIGRATE's datasimulator (migdata).)\n");
 fprintf(stderr,"\t -q sites   ((to put the sites numbers correctly into migrate output)\n");
	...
	double scaling;
{
	double t;
	int i, *descl, *descr ;
	void parens( struct node *ptree, int *descl, int *descr, int noden, double scaling );
	...
	parens( ptree, descl, descr, 2*nsam-2, scaling);
	free( descl ) ;
	free( descr ) ;
}

	void
	parens( struct node *ptree, int *descl, int *descr,  int noden, double scaling)
{
	double time ;

   if( descl[noden] == -1 ) {
     printf("%s_%d:%10.10lf", (ptree+noden)->name, noden+1, (ptree+ ((ptree+noden)->abv))->time*scaling);
	}
   else{
	printf("(");
	parens( ptree, descl,descr, descl[noden], scaling ) ;
	printf(",");
	parens(ptree, descl, descr, descr[noden], scaling ) ;
	if( (ptree+noden)->abv == 0 ) printf(");\n"); 
	else {
	  time = (ptree + (ptree+noden)->abv )->time - (ptree+noden)->time ;
	  printf("):%10.10lf", time*scaling);
	  }
        }
}
